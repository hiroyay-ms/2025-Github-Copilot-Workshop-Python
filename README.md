...
- 🎨 **視覚的フィードバック**: 
  - 円形プログレスバーの色変化アニメーション（青→黄→赤）
  - 背景エフェクト（パーティクルと波紋アニメーション）
- ⚙️ **カスタマイズ設定**:
  - 作業時間・休憩時間の調整（5分〜60分）
  - テーマ切り替え（ライト/ダーク/フォーカス）
  - サウンド設定（開始音/終了音/ティック音）
- 📊 **統計表示**: 今日の完了セッション数と総集中時間
- 🏆 **ゲーミフィケーション**: バッジシステム、レベルアップ、週間統計
...
| GET | `/api/settings` | 設定取得 | - | `{"work_duration": 25, "break_duration": 5, ...}` |
| POST | `/api/settings` | 設定保存 | `{"work_duration": 25, ...}` | `{"work_duration": 25, ...}` |
| GET | `/api/gamification/profile` | プロフィール取得 | - | `{"level": 5, "exp": 250, ...}` |
| GET | `/api/gamification/badges` | バッジ一覧取得 | - | `[{"id": "...", "name": "..."}]` |
| GET | `/api/gamification/stats/weekly` | 週間統計取得 | - | `{"total_sessions": 20, ...}` |
...
3. **視覚的フィードバック**: 
   - 円形プログレスバーが徐々に減少し、色が変化（青→黄→赤）
   - 背景に金色のパーティクルと波紋エフェクトが表示
4. **セッション完了**: タイマーが0になると音声通知とブラウザ通知が表示されます
4. **セッション完了**: タイマーが0になると音声通知とブラウザ通知が表示されます
5. **統計確認**: 画面下部に今日の完了セッション数と総集中時間が表示されます
6. **リセット**: 途中でやめたい場合は「リセット」ボタンをクリック

### カスタマイズ設定

画面右上の⚙️アイコンをクリックすると設定モーダルが開きます：

1. **タイマー設定**
   - 作業時間: 5分〜60分（デフォルト: 25分）
   - 休憩時間: 1分〜30分（デフォルト: 5分）

2. **テーマ設定**
   - **ライト**: 青紫グラデーション背景（デフォルト）
   - **ダーク**: 暗い背景でナイトモード
   - **フォーカス**: シンプルな白背景で集中モード
...
作業セッション中、以下の視覚効果が自動的に表示されます：

- **プログレスバーの色変化**: 時間経過に応じて青→黄色→赤へグラデーション
- **背景パーティクル**: 20個の金色パーティクルが浮遊
- **波紋エフェクト**: 3つの同心円が画面全体に広がる

これらのエフェクトは`prefers-reduced-motion`に対応しており、アニメーション軽減設定を有効にしている場合は自動的に最小化されます。
